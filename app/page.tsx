...
export default function Home() {
  ...
  }, [letterOpen, playSfx, postLetterMode]);
  ...
  const resolveFinaleChoice = useCallback(
    (choice: 'stars' | 'lanterns' | 'blooms' | 'random') => {
      if (choice !== 'random') return choice;
      const options: Array<'stars' | 'lanterns' | 'blooms'> = ['stars', 'lanterns', 'blooms'];
      return options[Math.floor(Math.random() * options.length)];
    },
    []
  );

  const runFinaleSequence = useCallback(
    (choice: 'stars' | 'lanterns' | 'blooms' | 'random') => {
      if (finaleRunning) return;
      clearFinaleChoiceTimers();
      setFinaleRunning(true);
      const selected = resolveFinaleChoice(choice);
      const isLowPerf = reducedMotion || perfTier === 'low';
      const rect =
        boundsRef.current ??
        containerRef.current?.getBoundingClientRect() ??
        new DOMRect(0, 0, viewport.width || 800, viewport.height || 600);
      ...
      if (selected === 'stars') {
        const duration = isLowPerf ? 4200 : 6200;
        const minDelay = isLowPerf ? 650 : 350;
        const maxDelay = isLowPerf ? 950 : 500;
        const startAt = performance.now();
        playSfx(sparkleRef, { volume: 0.28, rateMin: 0.98, rateMax: 1.02 });
        addSparkle(centerX, centerY, isLowPerf ? 0.8 : 1.2);
        const scheduleStar = () => {
          if (performance.now() - startAt >= duration) {
            finish();
            return;
          }
          addShootingStar(isLowPerf ? 1.6 : 2.2);
          const delay = minDelay + Math.random() * (maxDelay - minDelay);
          finaleChoiceTimersRef.current.push(window.setTimeout(scheduleStar, delay));
        };
        scheduleStar();
        finaleChoiceTimersRef.current.push(window.setTimeout(finish, duration + 300));
        return;
      }

      if (selected === 'lanterns') {
        const duration = isLowPerf ? 4800 : 7000;
        const minDelay = isLowPerf ? 900 : 500;
        const maxDelay = isLowPerf ? 1200 : 760;
        const startAt = performance.now();
        playSfx(sparkleRef, { volume: 0.22, rateMin: 0.97, rateMax: 1.01 });
        addAura(centerX, centerY);
        const scheduleLantern = () => {
          if (performance.now() - startAt >= duration) {
            finish();
          ...
          addLantern();
          const delay = minDelay + Math.random() * (maxDelay - minDelay);
          finaleChoiceTimersRef.current.push(window.setTimeout(scheduleLantern, delay));
        };
        scheduleLantern();
        finaleChoiceTimersRef.current.push(window.setTimeout(finish, duration + 400));
        return;
      }

      const duration = isLowPerf ? 5200 : 6800;
      const waveCount = isLowPerf ? 2 : 3;
      const seedsPerWave = isLowPerf ? 3 : 5;
      const maxRadius = Math.min(rect.width, rect.height) * 0.22;
      playSfx(sparkleRef, { volume: 0.24, rateMin: 0.97, rateMax: 1.02 });
      addPetalBurst(centerX, centerY, isLowPerf ? 1 : 1.4);

      const triggerWave = (waveIndex: number) => {
        const ringRadius = maxRadius * (0.45 + waveIndex * 0.25);
        for (let i = 0; i < seedsPerWave; i += 1) {
          const angle = (Math.PI * 2 * i) / seedsPerWave + waveIndex * 0.4;
          const seedX = centerX + Math.cos(angle) * ringRadius;
          const seedY = centerY + Math.sin(angle) * ringRadius;
          plantAt(seedX, seedY, rect);
          addPetalBurst(seedX, seedY, isLowPerf ? 0.8 : 1.1);
        }
      };

      for (let wave = 0; wave < waveCount; wave += 1) {
        const delay = wave * (isLowPerf ? 1200 : 900);
        finaleChoiceTimersRef.current.push(window.setTimeout(() => triggerWave(wave), delay));
      }

      finaleChoiceTimersRef.current.push(window.setTimeout(finish, duration));
    },
    [
      addAura,
      addLantern,
  ...
  const pressAt = (seedX: number, seedY: number, rect: DOMRect) => {
